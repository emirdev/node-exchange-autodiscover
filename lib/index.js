...
var EWS_URL_SETTING = 'ExternalEwsUrl';
...
function queryDns(domain) {
  return dnsResolve('_autodiscover._tcp.' + domain, 'SRV').then(function (response) {
...
function wrapSettingsRequest(settings) {
  ...
  if (settings.indexOf(EWS_URL_SETTING) === -1) settings.push(EWS_URL_SETTING);
  return settings.map(function (setting) {
    return '<a:Setting>' + setting + '</a:Setting>';
  }).join('');
};

function createAutodiscoverSoap(emailAddress, settings) {
  return '' + '<?xml version="1.0" encoding="utf-8"?>' + '<soap:Envelope xmlns:a="http://schemas.microsoft.com/exchange/2010/Autodiscover" ' + 'xmlns:wsa="http://www.w3.org/2005/08/addressing" ' + 'xmlns:xsi="http://www.w3.org/2001/XMLSchema-instance" ' + 'xmlns:soap="http://schemas.xmlsoap.org/soap/envelope/">' + '  <soap:Header>' + '    <a:RequestedServerVersion>Exchange2010</a:RequestedServerVersion>' + '    <wsa:Action>http://schemas.microsoft.com/exchange/2010/Autodiscover/Autodiscover/GetUserSettings</wsa:Action>' + '  </soap:Header>' + '  <soap:Body>' + '    <a:GetUserSettingsRequestMessage xmlns:a="http://schemas.microsoft.com/exchange/2010/Autodiscover">' + '      <a:Request>' + '        <a:Users>' + '          <a:User>' + '            <a:Mailbox>' + emailAddress + '</a:Mailbox>' + '          </a:User>' + '        </a:Users>' + '        <a:RequestedSettings>' + wrapSettingsRequest(settings) + '</a:RequestedSettings>' + '      </a:Request>' + '    </a:GetUserSettingsRequestMessage>' + '  </soap:Body>' + '</soap:Envelope>';
}
...
function autodiscoverDomains(domains, emailAddress, password, username, settings) {
  ...
  return Promise.any(promises).then(xmlToJson).then(function (result) {
    var userSettings = result.envelope.body.getUserSettingsResponseMessage.response.userResponses.userResponse.userSettings.userSetting;
    // Make sure we're working with an array
    userSettings = [].concat(userSettings);
    return userSettings.reduce(function (userSettings, setting) {
...
module.exports = function (params, cb) {
  ...
  return promise.then(function (otherDomains) {
